// Curried overload types for src/safe.ts functions.
// Overload order: curried forms first, full-arity LAST.
// This ensures Parameters<T>[N] resolves correctly for flip().

import type { AnyObject, Cond, AnyFunc, IndexesOfArray, Reducer } from './base'
...
export type T_join = {
  (delim: string): (arr: ArrayLike<any> & {join: AnyFunc<string, [delim: string]>}) => string
  (delim: string, arr: ArrayLike<any> & {join: AnyFunc<string, [delim: string]>}): string
}
...
export type T_memoize = {
  <Args extends any[]>
  (keyGen: (...args: Args) => string): (fn: AnyFunc<any, Args>) => (...args: Args) => any
  <Args extends any[]>
  (keyGen: (...args: Args) => string, fn: AnyFunc<any, Args>): (...args: Args) => any
}
...
export type T_zipObj = {
  <T1>(a: T1[]): <T2>(b: T2[]) => {[k: string]: T2}
  <T1 extends string | number, T2>(a: T1[], b: T2[]): {[k: string]: T2}
}
